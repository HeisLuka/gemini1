    ...
    def __getattr__(self, name):
        ...
        def wrapper(path, *args, **kwargs):
            ...
            if is_virtual:
                ...
                # --- НАЧАЛО НОВОГО БЛОКА: ШУНТИРОВАНИЕ ДЛЯ 'JOIN' ---
                if name == 'join':
                    import re
                    # 1. Позволяем оригинальной функции сделать свою работу
                    original_result = func(path, *args, **kwargs)
                    
                    # 2. Приводим все к единому стандарту (прямые слэши)
                    normalized_path = original_result.replace('\\', '/')
                    
                    # 3. "Схлопываем" двойные и более слэши в один,
                    #    но игнорируем '://' в URL-ах (например, http://, mem://).
                    #    (?<!:) - это "негативный просмотр назад", он проверяет, что перед слэшами нет двоеточия.
                    cleaned_path = re.sub(r'(?<!:)/+', '/', normalized_path)
                    
                    return cleaned_path
                # --- КОНЕЦ НОВОГО БЛОКА ---
                
                # Для всех остальных функций (basename, dirname и т.д.) вызываем как есть
...
def _install_qt_message_handler():
    """
    Устанавливает продвинутый перехватчик сообщений Qt.
    Ловит: таймеры, смену Parent, проблемы потоков и QObject.
    """
    ...
    def qt_message_handler(mode, context, message):
        ...
        # Печатаем стандартное сообщение в stderr, чтобы оно не терялось
        print(f"[Qt {mode_str}] {message}", file=sys.stderr)

        # 2. Проверяем, является ли это ошибкой многопоточности
        msg_lower = message.lower()
        is_threading_error = (
            "thread" in msg_lower and 
            any(word in msg_lower for word in ["timer", "parent", "qobject"])
        )

        if is_threading_error:
            # Получаем текущий стек вызовов для анализа
            stack = traceback.extract_stack()
            stack_names = [frame.name for frame in stack]

            # Исправленный "странный момент": корректная проверка на зацикливание
            if "_patched_qmessagebox_critical" in stack_names:
                return

            # 3. Красивый вывод отчета
            print("\n" + "!"*80, file=sys.stderr)
            print(f"🕵️ [SHERLOCK] ОБНАРУЖЕНА ОШИБКА ПОТОКОВ!", file=sys.stderr)
            print(f"   Тип: {mode_str}", file=sys.stderr)
            print(f"   Сообщение Qt: {message}", file=sys.stderr)
            print("-" * 80, file=sys.stderr)
            print("   ПУТЬ В ПРИЛОЖЕНИИ (Traceback):", file=sys.stderr)
            
            # Печатаем стек, исключая сам обработчик для чистоты
