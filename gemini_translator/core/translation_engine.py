...
from ..core.workers import UniversalWorker
...
class TranslationEngine(QObject):
    ...
    def __init__(self, context_manager=None, settings_manager=None, task_manager=None, parent=None, event_bus=None):
        ...
        self.ramp_up_timer = QtCore.QTimer()
        self.session_monitor_timer = QtCore.QTimer()
    ...
    def on_event(self, event: dict):
        ...
        if event_name == 'temporary_limit_warning_received' and 'worker' in source:
            now = time.time()
            last_time = self.last_warning_times.get(worker_key, 0)
            if now - last_time < 1.0:
                return 
            self.last_warning_times[worker_key] = now
            ...
            if has_plenty_resource:
                ...
                has_bad_history = self.key_warning_counters.get(worker_key, 0) >= 2
            else:
                ...
                has_bad_history = self.key_warning_counters.get(worker_key, 0) >= (0.3 * self.MAX_REPEATED_WAITS)
            # ------------------------------------------
            ...
            if is_long_pause and has_bad_history:
                ...
                elif self.key_warning_counters.get(worker_key, 0) >= self.MAX_REPEATED_WAITS:
    ...
    def _finalize_scheduled_launch(self, key):
    ...
    def _apply_warning_penalty(self, worker_id: str, model_id: str = "Unknown",
                            penalty_points: int = 1, worker_session=None) -> bool:
        """
        ...
        # ВАЖНО: выбираем model_id для подсчета корректно
        model_id_for_count = None
        if model_id and model_id != "Unknown":
            model_id_for_count = model_id
        else:
            model_id_for_count = getattr(self, "model_id", None)

        if rpd_limit > 0:
            key_info = self.settings_manager.get_key_info(worker_key)
            if key_info:
                # Если model_id_for_count None, get_request_count может вернуть 0
                # => защита RPD будет бесполезна. Лучше явно отработать этот случай.
                if not model_id_for_count:
                    self._post_event('log_message', {
                        'message': f"[MANAGER-RPD] ⚠️ Не удалось определить model_id для подсчета расхода ключа …{worker_key[-4:]}."
                    })
                else:
                    current_count = self.settings_manager.get_request_count(key_info, model_id_for_count)
                    ...
                    if current_count >= threshold:
                        ...
                        fake_exception = Exception(
                            f"RPD Risk Threshold triggered ({current_count} >= {int(threshold)})."
                        )

                        payload = {"type": "quota_exceeded", "model_id": model_id_for_count, "exception": fake_exception}
                        event = {
                            'event': "fatal_error",
        ...
        # ВАЖНО: штрафы копим по КЛЮЧУ, а не по worker_id
        counter_key = worker_key
        current_warnings = self.key_warning_counters.get(counter_key, 0) + penalty_points
        self.key_warning_counters[counter_key] = current_warnings
        ...
        if current_warnings >= self.MAX_REPEATED_WAITS:
            ...
            payload = {"type": "quota_exceeded", "model_id": model_id_for_count or model_id, "exception": fake_exception}
    ...
    def _reset_key_warning_counter(self, worker_id: str):
        ...
        if self.key_warning_counters.get(worker_key, 0) > 0:
            self.key_warning_counters[worker_key] = 0
            self._post_event('log_message', {
                'message': f"[MANAGER] ✅ Ключ …{worker_key[-4:]} подтвердил работоспособность. Счетчик предупреждений сброшен."
            })

    def _handle_fatal_error(self, worker_id: str, payload: dict, worker_session=None):
        if worker_id not in self.active_workers_map:
            self._post_event('log_message', {'message': f"[FATAL][IGNORED] worker …{worker_id[-4:]} не найден в active_workers_map"})
            return
        if worker_id in self.shutting_down_workers:
            self._post_event('log_message', {'message': f"[FATAL][IGNORED] worker …{worker_id[-4:]} уже в shutting_down_workers"})
            return
    ...
    def apply_and_start_session(self, settings: dict):
        ...
        num_instances = settings.get('num_instances', 1)
        total_session_keys = settings.get('api_keys', [])
        max_pool_size = len(total_session_keys) if total_session_keys else num_instances
        ...
        self.api_key_manager = ApiKeyManager(settings.get('api_keys', []))
    ...
    def _launch_next_from_ramp_up(self):
        ...
        if key_to_launch:
            ...
            self.ramp_up_timer.stop() # Ключи кончились
    ...
    def _launch_worker(self, api_key: str):
        ...
        if self.is_cancelled or self.is_session_finishing or (current_worker_id and current_worker_id in self.active_workers_map):
            return
    ...
    def _try_launch_replacement(self):
        if self.is_cancelled or self.is_session_finishing:
        ...
        if key_to_launch:
            ...
            QtCore.QTimer.singleShot(self.RAMP_UP_INTERVAL_MS, 
                                     lambda: self._finalize_scheduled_launch(key_to_launch))
            return True
    ...
    def _stop_timers(self):
        ...
        if hasattr(self, 'ramp_up_timer') and self.ramp_up_timer.isActive():
            self.ramp_up_timer.stop()
        if hasattr(self, 'session_monitor_timer') and self.session_monitor_timer.isActive():
            self.session_monitor_timer.stop()
        if not self.is_cancelled and not self.is_session_finishing:
    ...
    def log_thread_identity(self):
        ...
        print(f"\n[SYSTEM] ⚙️ ENGINE THREAD ID: {ident}\n")
