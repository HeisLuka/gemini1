...
class GlossaryBatchProcessor(BaseTaskProcessor):
    async def execute(self, task_info, use_stream=False):
        ...
        try:
            ...
            context_glossary_data = task_payload[3:]
        except IndexError:
        ...
        json_text = ""
        try:
            raw_response = await self.worker.api_handler_instance.execute_api_call(user_prompt, log_prefix, allow_incomplete=True, use_stream=use_stream)
            json_text = clean_html_content(raw_response)
        except PartialGenerationError as e:
            self.worker._post_event('log_message', {'message': f"[WARN] Ответ JSON оборван. Запускаю протокол 'Феникс'..."})
            partial_clean = clean_html_content(e.partial_text)
            repaired_json = repair_json_string(partial_clean)
            if repaired_json:
                self.worker._post_event('log_message', {'message': f"[SUCCESS] 'Феникс' восстановил валидную часть JSON!"})
                json_text = repaired_json
            else:
                raise ValidationFailedError(f"Не удалось получить JSON из частичного ответа API. Исходная ошибка: {e}")
        ...
        try:
            ...
            try:
                ...
                if parsed_glossary_dict is None:
                    raise ValidationFailedError(f"Не удалось восстановить JSON. Ошибка: {e}")

            if not isinstance(parsed_glossary_dict, dict):
                raise ValidationFailedError("Ответ от AI не является словарем JSON.")
            ...
            for original, value in parsed_glossary_dict.items():
                if not isinstance(original, str) or not isinstance(value, dict): continue
                raw_rus = value.get("rus") or value.get("translation") or ""
                if not raw_rus: continue
                rus = str(raw_rus).replace('—', '–')
                note = str(value.get("note", "")).replace('—', '–')
                if not force_accept and not re.search(r'[а-яА-ЯёЁ]', rus) and not re.search(r'[а-яА-ЯёЁ]', note): continue
            ...
            if not force_accept:
                ...
                ai_glossary_as_dict = {item['original']: {'rus': item['rus'], 'note': item['note']} for item in pre_validated_glossary_list if item.get('original')}
                ...
                truly_validated_glossary_list = [term_data for term_data in pre_validated_glossary_list if term_data.get('original') in found_terms_dict]
                discarded_count = len(pre_validated_glossary_list) - len(truly_validated_glossary_list)
                if discarded_count > 0: self.worker._post_event('log_message', {'message': f"🔎 [Глоссарий] Отфильтровано {discarded_count} терминов (нет в тексте)."})
            else:
            ...
            if truly_validated_glossary_list and new_terms_limit and new_terms_limit > 0:
                ...
                for term in truly_validated_glossary_list:
                    if term.get('original') in existing_originals_set: updated_terms.append(term)
            ...
            if truly_validated_glossary_list:
                ...
                final_msg = ""
                if merge_mode == 'update': final_msg = f"✅ …{wid_short}: Обработано {stats['total']}. Обновлено: {stats['updated']}, Новых: {stats['new']}."
                elif merge_mode == 'supplement':
                    updated_count = num_updated_override if num_updated_override is not None else stats['updated']
                    if stats['new'] > 0: final_msg = f"✅ …{wid_short}: Найдено {stats['new']} новых терминов (дубликатов: {updated_count})."
                    else: final_msg = f"✅ …{wid_short}: Новых терминов не найдено (все {updated_count} уже были в базе)."
                else: final_msg = f"✅ …{wid_short}: Записано {stats['total']} терминов (уникальных: {stats['new']})."
            ...
            raise ValidationFailedError(f"Не удалось распарсить или обработать JSON: {e}")
