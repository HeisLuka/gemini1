    ...
    def prepare_for_api(self, text_content, system_instruction_text, completion_data=None, current_chapters_list=None):
        ...
        if completion_data:
            ...
            DEFAULT_COMPLETION_INSTR = (
                "\n---\n### ЗАДАЧА: ЗАВЕРШЕНИЕ ПРЕРВАННОГО ОТВЕТА ###\n"
                "**ИНСТРУКЦИЯ:** Текущая задача — вернуть **ТОЛЬКО НЕДОСТАЮЩУЮ ЧАСТЬ** от полного и цельного текста, чтобы он бесшовно продолжил уже существующий фрагмент.\n"
                "\n--- ПРЕРВАННЫЙ ОТВЕТ:\n\n```html\n{partial_translation}"
            )
