...
class UniversalWorker(QObject):
    ...
    def __init__(self, **kwargs):
        ...
        # 2. Инструктаж (сначала определяем, кто мы такие)
        ...
        # Полная настройка состояния
        self._initialize_worker_state(kwargs)
        
        # 3. Выход на связь (только когда всё готово!)
        ...
        try:
            ...
            self._post_event('log_message', {'message': error_msg})
                    ...
                    if loop and not loop.is_closed():
                        try:
                            # Пытаемся закрыть сессию корректно (с асинхронным ожиданием)
                            loop.run_until_complete(self.api_handler_instance._close_thread_session_internal())
                        except Exception as e:
                            print(f"[CLEANUP ERROR] Не удалось штатно закрыть сессию: {e}")
                        finally:
                            # В любом случае закрываем сам event loop
                            # Это высвобождает системные ресурсы потока
                            loop.close()
                    
                    self.cancel()
                    print(f"[WORKER LIFECYCLE] Поток {threading.get_native_id()} ЗАВЕРШИЛ РАБОТУ.")
            ...
            if is_requeued:
                ...
                if success:
                    ...
                    if task_type in ('epub_chunk', 'raw_text_translation'):
                        ...
                        self.task_manager.task_done(self.worker_id, task_info_for_event)
                else:
