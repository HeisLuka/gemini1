    ...
    def setup_client(self, client_override=None, proxy_settings=None):
        ...
        if not is_localhost and self.proxy_settings and self.proxy_settings.get('enabled'):
            ...
            if host and port:
                ...
                print(f"[LocalApiHandler] Прокси настроен для удаленного сервера: {url}")
        elif is_localhost:
             print(f"[LocalApiHandler] Обнаружен локальный адрес. Прокси принудительно отключен.")
