...
def finalize_cleanup(html_content: str) -> str:
    ...
    tag_map = {}
    def mask_callback(m):
        key = f"\0TAG_FIN_{len(tag_map)}\0"
        tag_map[key] = m.group(0)
        return key
...
def initial_cleanup(html_content: str) -> str:
    ...
    tag_map = {}
    def mask_callback(m):
        key = f"\0TAG_INIT_{len(tag_map)}\0"
        tag_map[key] = m.group(0)
        return key

    masked_content = re.sub(r'<[^>]+>', mask_callback, content)
    ...
    for key, val in tag_map.items():
        masked_content = masked_content.replace(key, val)
...
def prettify_html(html_content: str) -> str:
    ...
    # СКРЫВАЕМ ТЕГИ (И АТРИБУТЫ В НИХ)
    content = re.sub(r'<[^>]+>', mask_callback, content)
