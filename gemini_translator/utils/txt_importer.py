...
# --- CJK maps ---
_CJK_DIGITS = {"零":0,"〇":0,"一":1,"二":2,"三":3,"四":4,"五":5,"六":6,"七":7,"八":8,"九":9,"两":2,"兩":2}
_CJK_UNITS  = {"十":10,"百":100,"千":1000}
_CJK_BIG    = {"万":10000,"萬":10000,"亿":100000000,"億":100000000}

def _cjk_to_int(cjk_s: str) -> Optional[int]:
    if not cjk_s:
        return None
    cjk_s = unicodedata.normalize("NFKC", cjk_s).strip()
    if not cjk_s:
        return None
    if cjk_s.isdigit():
        try:
    ...
    section += number
    return total + section  # может быть 0, если "零"
...
def _english_words_to_int(text: str) -> Optional[int]:
    if not text:
        return None
    t = unicodedata.normalize("NFKC", text).lower().replace("-", " ")
    # выкинем пунктуацию, оставим слова/пробелы
    t = re.sub(r"[^a-z\s]", " ", t)
    tokens = [w for w in t.split() if w and w not in ("and",)]
    ...
    for w in tokens:
        if w in ("chapter", "ch", "chap", "episode", "part", "book", "volume", "vol"):
            continue
        if w in _EN_UNITS:
            current += _EN_UNITS[w]
            saw = True
...
def _looks_like_title_line(s: str) -> bool:
    """
    Быстрая эвристика: строка похожа на заголовок главы (а не дата/мусор).
    """
    if not s:
        return False
    ss = s.strip()
    ...
    # выкинуть URL/проценты/время/даты
    if "http://" in low or "https://" in low:
        return False
    if re.search(r"\b\d{4}[-/\.]\d{1,2}[-/\.]\d{1,2}\b", low):  # 2024-01-12
        return False
    if re.search(r"\b\d{1,2}:\d{2}\b", low):  # 12:34
        return False
    if "%" in low:
        return False
...
def smart_extract_number_from_title(title: str) -> Optional[int]:
    """
    Умное извлечение номера главы из заголовка.

    Приоритет (детерминированный):
    1) Маркеры + арабские цифры: Chapter/Ch/Episode/Vol/... + 123
    2) CJK: 第123章 / 第123回 / 第123节
    3) CJK словами: 第十二章 -> через _cjk_to_int (без зависимостей)
    4) Римские: Chapter IX -> _roman_to_int
    5) English words: Chapter Twenty One -> _english_words_to_int
    6) Microsoft Recognizers (если доступны) как запасной вариант
    7) Fallback: просто число, но только если строка похожа на заголовок (_looks_like_title_line)
    """
    if not title:
        return None
    ...
    # 1) Chapter/Ch/Episode/Part/Vol + digits
    m = re.search(r"(?i)\b(?:chapter|ch\.?|chap\.?|episode|part|vol\.?|volume|book)\b\D{0,6}(\d{1,8})\b", s)
    if m:
        try:
    ...
    # 2) CJK digits: 第 123 章/回/节/節 (суффикс опционален)
    m = re.search(r"第\s*(\d{1,8})\s*(?:章|回|节|節)?\b", s)
    if m:
        try:
            return int(m.group(1))
        except Exception:
    ...
    # Korean "제 12 장" digits (и/или без суффикса)
    m = re.search(r"제\s*(\d{1,8})\s*(?:장|편)?\b", s)
    if m:
        try:
            return int(m.group(1))
        except Exception:
    ...
    # 3) CJK словами: 第十二章 / 第一百二十回
    m = re.search(r"第\s*([0-9零〇一二三四五六七八九十百千万兩两萬亿億]+)\s*(?:章|回|节|節)?\b", s)
    if m:
        v = _cjk_to_int(m.group(1))
        if v is not None:
            return v

    # Korean words в стиле "제십이장" встречается реже — обычно digits. (Если надо — добавим отдельно.)

    # 4) Римские после Chapter/Ch
    m = re.search(r"(?i)\b(?:chapter|ch\.?|chap\.?)\b\W*([ivxlcdm]{1,12})\b", s)
    if m:
        v = _roman_to_int(m.group(1))
        if v is not None:
            return v

    # 5) English words после Chapter/Ch
    m = re.search(r"(?i)\b(?:chapter|ch\.?|chap\.?)\b\W*([a-z][a-z\s-]{0,60})", s)
    if m:
        v = _english_words_to_int(m.group(1))
        if v is not None:
            return v

    # 6) Microsoft Recognizers (план Б)
    if 'HAS_RECOGNIZERS' in globals() and HAS_RECOGNIZERS:
        try:
            results = []
            for cult in [Culture.Chinese, Culture.Japanese, Culture.English, Culture.Korean, Culture.Russian]:
                try:
                    results.extend(recognize_number(s, cult))
                except Exception:
                    pass
            if results:
                leftmost = min(results, key=lambda x: getattr(x, "start", 10**9))
                v = getattr(leftmost, "resolution", {}).get("value")
                if v is not None:
                    # иногда прилетает "12" или "12.0"
                    return int(float(v))
        except Exception:
            pass

    # 7) Fallback: просто число, но только если строка похожа на заголовок
    if _looks_like_title_line(s):
        m = re.search(r"(?<!\d)(\d{1,6})(?!\d)", s)   # ← работает для "章12", "第12章" и т.д.
        if m:
            try:
                return int(m.group(1))
            except Exception:
...
class TxtChapterAnalyzer:
    ...
    def analyze_potential_markers(self):
        ...
        cjk_di_pat  = re.compile(rf'^(第)\s*({cjk_num_regex})\s*([章节回]?)')
        cjk_zhan_pat = re.compile(rf'^([章节回卷])\s*({cjk_num_regex})')
        indent_hits = 0
        ...
        for i in sample_indices:
            ...
            cjk_match = cjk_di_pat.match(line_l) or cjk_zhan_pat.match(line_l)
            if cjk_match:
                suffix = cjk_match.group(3) if cjk_di_pat.match(line_l) else ""
                prefix = cjk_match.group(1)
                key = f"{prefix}...{suffix}" if suffix else f"{prefix}..."
        ...
        candidates.add(r'^[章卷回节]\s*\d+')  # Универсальная база
        candidates.add(r'^(?i)chapter\s*\d+')
        candidates.add(r'^(?i)глава\s*\d+')
        
        for p, _ in numeric_counter.most_common(5):
            prefix_char = p.split("...")[0]
            sfx = p.split("...")[-1]
            if prefix_char == "第":
                candidates.add(rf'^第\s*{cjk_num_regex}\s*{re.escape(sfx)}' if sfx
                            else rf'^第\s*{cjk_num_regex}')
            else:
                # 章N, 回N, 节N, 卷N и т.д.
                candidates.add(rf'^{re.escape(prefix_char)}\s*{cjk_num_regex}')
        for word, count in text_starts.most_common(5):
            if count >= 2: candidates.add(rf'^{re.escape(word)}\s')

        enriched = []

        # 1. Спец-маркер: Отступы (всегда получает высокий балл, если подходит)
        if potential_headers_in_sample > 0 and (indent_hits / potential_headers_in_sample) > 0.4:
            ...
            if score > 0:
                enriched.append({
                    ...
                    'uniq_cnt': info['count'],
                    'hits': info['count'],
                    'tuple': ("Контекст (Нет отступа перед 　　)", "__CONTEXT_INDENT_ANALYSIS__", "context_indent"),
                    'name': "Контекст (Нет отступа перед 　　)"
                })

        # 2. Прогон всех кандидатов через ДВОЙНОЙ фильтр (Конвейер)
        for pat in candidates:
            try:
                fast_reg = re.compile(pat, re.IGNORECASE)
                
                # Собираем точные совпадения и их индексы
                matched_lines = []
                indices = []
                for idx, line in enumerate(self.lines):
                    clean = line.strip()
                    if fast_reg.match(clean):
                        indices.append(idx)
                ...
                # ФАЗА 1: "Тупая" физика (Режет мусор, который не делит текст на блоки)
                phys_score, info = self._validate_indices(indices)
                if phys_score <= 0:
                    continue
                    
                # ФАЗА 2: "Умная" лингвистика (Передаем только совпавшие строки, экономим ресурсы!)
                sem_score, count_uniq = self._smart_score_matched_lines(matched_lines, pat)
                if sem_score < 0:
                    continue # Лингвистика забраковала (например, нет цифр)
                    
                total_score = phys_score + sem_score
                ...
                enriched.append({
                    ...
                    'uniq_cnt': count_uniq,
                    'hits': info['count'],
                    'tuple': (display, pat, 'direct_regex'),
                    'name': display
                })
            except Exception as e:
                continue

        # Сортируем: Score ↑, Uniq ↑, Hits ↓
        enriched.sort(key=lambda x: (-x['score'], -x['uniq_cnt'], x['hits']))
        return enriched[:30]
    ...
    def _smart_score_matched_lines(self, matched_strings, rx):
        """
        Молниеносный умный скоринг. Работает ТОЛЬКО с найденными строками.
        Возвращает: (score, count_uniq). Если мусор — возвращает score < 0.
        """
        hits = len(matched_strings)
        nums = []
        uniq_nums = set()
        
        chapter_suffix_re = re.compile(r"(章|回|节|節|卷|篇|话|話|集|部)")
        chapter_word_re = re.compile(r"(?i)\b(chapter|ch\.?|chap\.?|episode|part|vol\.?|volume|book|глава)\b")
        anti_suffix_re = re.compile(
            r"第\s*[0-9零〇一二三四五六七八九十百千万兩两萬亿億]+\s*"
            r"(次|步|个|個|位|种|種|条|條|项|項|天|年|月|日|周|週|"
            r"分钟|分鐘|小时|小時|秒|页|頁|米|公斤|名|人)"
            r"(?=$|[\s，,。．.!！?？:：;；、\)\]】」』”\"'<>])"
        )
        anti_prefix_re = re.compile(r"^第[一二三四五六七八九十两兩0-9]+(?:次|步|个|個|位|种|種|条|條|项|項)")

        good_hits = 0
        bad_hits = 0
        anti_hits = 0

        for ln in matched_strings:
            has_chapter_marker = bool(chapter_suffix_re.search(ln) or chapter_word_re.search(ln))
            if has_chapter_marker:
                good_hits += 1
            else:
                bad_hits += 1

            if (not has_chapter_marker) and (anti_suffix_re.search(ln) or anti_prefix_re.search(ln)):
                anti_hits += 1

            n = smart_extract_number_from_title(ln)
            if n is not None:
                nums.append(n)
                uniq_nums.add(n)

        count_uniq = len(uniq_nums)
        
        # Строгий фильтр: если это не отступы, и мы ищем регулярками,
        # в заголовке ОБЯЗАНЫ быть хоть какие-то номера. Иначе это имена или диалоги!
        if not nums:
            return -100, 0
            
        score = int(20 * math.log1p(hits))
        
        uniq = sorted(list(uniq_nums))
        if count_uniq >= 3:
            span = uniq[-1] - uniq[0] + 1
            density = count_uniq / max(1, span)
            score += int(100 * density)
            is_sequential = all(uniq[i + 1] - uniq[i] == 1 for i in range(count_uniq - 1))
            if is_sequential:
                score += 50
        else:
            score += 10
        ...
        nums_len = len(nums)
        junk_hits = hits - nums_len
        if junk_hits > 0:
            score -= int(100 * (junk_hits / hits))
            
        rx_requires_suffix = bool(re.search(r"(章|回|节|節|卷|篇|话|話|集|部)", rx))
        if bad_hits > 0 and not rx_requires_suffix:
            score -= int(100 * (bad_hits / hits))
            
        if anti_hits > 0 and not rx_requires_suffix:
            score -= int(300 * (anti_hits / hits))
        ...
        k = len(nums)
        if k >= 3:
            score += int(100 * (inc / (k - 1)))
            score -= int(200 * (dec / (k - 1)))
            score -= int(100 * (eq / (k - 1)))
            
        if anti_hits >= 5 and not rx_requires_suffix:
            score -= 100
            
        return score, count_uniq
...
class TxtImportWizardDialog(QDialog):
    ...
    def _run_structure_analysis(self):
        ...
        try:
            enriched_markers = self.analyzer.analyze_potential_markers() or []
        except Exception as e:
            print(f"[STRUCT] Ошибка подготовки данных: {e}")
            self._update_stats_from_regex_input()
            return

        if not enriched_markers:
            self._update_stats_from_regex_input()
            return

        for data in enriched_markers:
            # Читаем данные из нового конвейера
            label = f"{data['name']} [вхожд: {data['hits']}] | качество: {data['score']} | (№ {data['uniq_cnt']})"
            item = QListWidgetItem(label)
            item.setData(Qt.ItemDataRole.UserRole, data['tuple'])
            
            # Подсвечиваем самых надежных кандидатов
            if data['score'] > 200:
    ...
    def _update_stats_from_regex_input(self):
        rx = self.regex_input.text().strip()

        # 1) пусто
        if not rx:
            self._set_regex_ok(True)
            self._fill_stats({'count': 0, 'min_val': 0, 'max_val': 0, 'avg': 0})
            return
        ...
        try:
            ...
            self._set_regex_ok(False, str(e))
            self._fill_stats({'count': 'Ошибка', 'min_val': 0, 'max_val': 0, 'avg': 0})
            return

        self._set_regex_ok(True)
        stats = self.analyzer.calculate_stats(custom_regex=rx)
        self._fill_stats(stats)

    def _set_regex_ok(self, ok: bool, err: str = ""):
        palette = self.regex_input.palette()
        palette.setColor(
            QtGui.QPalette.ColorRole.Text,
            QtCore.Qt.GlobalColor.red if not ok else
            self.style().standardPalette().color(QtGui.QPalette.ColorRole.Text)
        )
        self.regex_input.setPalette(palette)
        self.regex_input.setToolTip("" if ok else f"Regex ошибка: {err}")

    def _fill_stats(self, stats: dict):
        self.stats_table.setItem(0, 1, QTableWidgetItem(f"{stats.get('count', 0):,}" if isinstance(stats.get('count'), int) else str(stats.get('count'))))
    ...
    def _promote_selection_to_regex(self, current, prev):
        ...
        name, value, mtype = data[0], data[1], data[2]

        self.regex_input.blockSignals(True)
        try:
            if mtype in ("direct_regex", "context_indent"):
                self.regex_input.setText(value)
                self.regex_input.setToolTip(f"Активный шаблон: {name}")
            else:
                # ВАЖНО: не очищаем поле, просто подсказка
                self.regex_input.setToolTip(f"Маркер '{name}' не является простым Regex.")
        finally:
