...
class MainWindow(QDialog):
    ...
    def __init__(self, parent=None, mode='standalone', project_path=None):
        ...
        # Если передан путь к проекту, пытаемся загрузить глоссарий
        if self.associated_project_path and self.launch_mode == 'standalone':
...
# ---------------------------------------------------------------------------
    ...
    def closeEvent(self, event):
        ...
        # 1. Проверка на несохраненные изменения (существующая логика через reject)
        # reject() сам вызовет super().reject() если все ок, но closeEvent перехватывает это раньше.
        # Поэтому здесь мы реализуем только логику навигации, 
        # а сохранение оставим на совести кнопок и явного вызова.
