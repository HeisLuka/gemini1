...
from PyQt6.QtWidgets import (
    ...
    QSpinBox, QProgressBar, QMessageBox, QDialogButtonBox, QTabWidget,
...
class GlossaryFrequencyWorker(QThread):
    """Фоновый рабочий для сканирования EPUB."""
    ...
    def __init__(self, epub_path, glossary_data, parent=None):
        ...
        self.glossary_dict = {
            entry.get('original', '').strip(): entry 
        ...
        try:
            ...
            with zipfile.ZipFile(self.epub_path, 'r') as zf:
                html_files = [
                    ...
                    if f.lower().endswith(('.html', '.xhtml', '.htm')) and not f.startswith('__MACOSX')
                ]
                total_files = len(html_files)
                
                for i, filename in enumerate(html_files):
                    if not self._is_running: break
                    self.progress_update.emit(i + 1, total_files, os.path.basename(filename))
                    try:
                        raw_content = zf.read(filename).decode('utf-8', errors='ignore')
                        clean_text = ""
                        if BS4_AVAILABLE:
                        ...
                        found_terms = regex_service.find_matches(clean_text)
                        for term in found_terms:
                            term_distribution[term].add(filename)
                    except Exception as e:
                        print(f"[FreqAnalyzer] Ошибка чтения {filename}: {e}")
                        continue
            
            if self._is_running:
                self.analysis_finished.emit(term_distribution)
        except Exception as e:
...
class TermFrequencyAnalyzerDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        self.threshold_spin.setRange(0, 50); self.threshold_spin.setValue(1); self.threshold_spin.setSuffix(" глав(ах)")
        ...
        freq_label = QLabel("Самые частые термины (Топ-500). Доступно редактирование перевода и примечаний.")
    ...
    def _start_analysis_flow(self):
        if not self.epub_path or not os.path.exists(self.epub_path):
            ...
            if path: self.epub_path = path
            else: self.reject(); return

        self.progress_group.setVisible(True)
        self.worker = GlossaryFrequencyWorker(self.epub_path, self.glossary_data)
    ...
    def _on_analysis_finished(self, distribution_map):
        self.progress_group.setVisible(False)
        self.content_widget.setVisible(True)
        
        # --- ЛОГИКА АГРЕГАЦИИ (Наследование популярности) ---
        found_terms = list(distribution_map.keys())
        all_glossary_terms = list(self.glossary_map.keys())
        
        for found_term in found_terms:
            files_where_found = distribution_map[found_term]
            for sub_term in all_glossary_terms:
                if len(sub_term) >= len(found_term): continue
                if sub_term in found_term:
                    distribution_map[sub_term].update(files_where_found)
        # ----------------------------------------------------

        self.raw_distribution = distribution_map
        
        self.term_counts = {}
        for term in all_glossary_terms:
            files_set = self.raw_distribution.get(term, set())
            self.term_counts[term] = len(files_set)
