...
class KeyManagementWidget(QWidget):
    ...
    def get_active_keys(self):
        return [self.active_keys_list.item(i).data(QtCore.Qt.ItemDataRole.UserRole) for i in range(self.active_keys_list.count())]
        ...
        for list_widget in [self.available_keys_list, self.active_keys_list]:
            for i in range(list_widget.count()):
                ...
                if item.data(QtCore.Qt.ItemDataRole.UserRole) == key_to_update:
                    key_short = f"…{key_to_update[-4:]}" if len(
                        key_to_update) > 4 else key_to_update
                    item.setText(f"{key_short} (Запросов: {new_count})")
                    return
