...
class ModelSettingsWidget(QGroupBox):
    ...
    def set_available_models(self, provider_id: str): # <-- Теперь принимает ID
        ...
        if self.model_combo.count() > 0:
            # После заполнения устанавливаем модель по умолчанию и инициируем событие
            self.set_default_model(api_config.default_model_name())
        
        self.model_combo.blockSignals(False)
        self._on_model_changed(0) # Вызываем по индексу, чтобы сработал для первого элемента
    ...
    def _on_model_changed(self, index: int):
        ...
        for spin_box in [self.rpm_spin, self.max_concurrent_spin, self.rpd_spin]: spin_box.blockSignals(True)
        ...
        for spin_box in [self.rpm_spin, self.max_concurrent_spin, self.rpd_spin]: spin_box.blockSignals(False)
