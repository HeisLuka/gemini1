...
class TaskManagementWidget(QWidget):
    ...
    def on_event(self, event_data: dict):
        """
        Слушает событие-пульс и ВСЕГДА запускает полную перерисовку.
        Также проверяет, нужно ли показать кнопку "Обработка Фильтра".
        """
        event_name = event_data.get('event')
        
        if event_name == 'task_state_changed':
            QtCore.QTimer.singleShot(20, self.redraw_ui) # 20 мс - небольшая, но надежная задержка
        
        # Кнопка "Обработка Фильтра" зависит от финальных статусов,
        # поэтому ее проверяем после завершения сессии.
        if event_name == 'session_finished':
            # С небольшой задержкой, чтобы дать redraw_ui отработать
    ...
    def update_list(self, tasks_data, original_filename):
        """
        Просто передает список задач и имя файла в дочерний виджет.
        """
        self.chapter_list_widget.update_list(tasks_data, original_filename)
    ...
    def _do_redraw(self):
        """Реальная логика перерисовки, вызываемая по таймеру."""
        app = QtWidgets.QApplication.instance()
        if not (app.engine and app.engine.task_manager):
            # Проверяем, существует ли еще виджет, прежде чем его обновлять
        ...
        if self and self.chapter_list_widget:
            ui_state_list = app.engine.task_manager.get_ui_state_list()
            
            # --- ФИЛЬТРАЦИЯ ПО КАТЕГОРИЯМ ОШИБОК ---
            ...
            if filter_key:
                ...
                for item in ui_state_list:
                    # item = (orig_path, status, details)
                    details = item[2]
                    errors_map = details.get('errors', {})
                    
                    # Если в истории задачи есть данная ошибка
            ...
            # --- ШУНТИРОВАНИЕ ---
            # После того как список гарантированно обновлен, проверяем статусы
            self.check_and_update_retry_button_visibility()
    ...
    def check_and_update_retry_button_visibility(self):
        """
        Сканирует ИСТИННОЕ состояние задач в TaskManager, ищет статус 'error_filter'
        и в зависимости от этого показывает или скрывает кнопку.
        """
        found_filtered = False
        app = QtWidgets.QApplication.instance()

        # 1. Получаем доступ к "медицинской карте" (TaskManager)
        if app.engine and app.engine.task_manager:
            # 2. Запрашиваем "анализы" - сырой список состояния
            ui_state_list = app.engine.task_manager.get_ui_state_list()
            
            # 3. Ищем нужный диагноз в сырых данных
            for _, status, details in ui_state_list:
                # Нас интересует не текст, а внутренний статус
                if status == 'error' and 'CONTENT_FILTER' in details.get('errors', {}):
                    found_filtered = True
                    break
        
        # 4. Устанавливаем видимость кнопки на основе точных данных
